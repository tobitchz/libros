...
export class PestaniaConfigPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.sub = this.afAuth.user.subscribe(u => {
      if (u) {
        this.email = u.email ?? '';
  ...
  async openTemaModal() {
    ...
    if (role === 'confirm' || role === 'confirmar') {
   ...
   */
  private async cargarUsuario(forceReload = false) {
    const user = await this.authService.obtenerUsuario();
    if (forceReload) await user?.reload();

    if (user) {
      this.email = user.email ?? '';
      this.nombreUsuario = user.displayName ?? (user.email ? user.email.split('@')[0] : '');
      this.fotoUrl = user.photoURL ?? localStorage.getItem('fotoPerfil');
      this.nombreEd = this.nombreUsuario;
    } else {
      this.email = '';
      this.nombreUsuario = '';
      this.fotoUrl = null;
      ...
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        {
          ...
          handler: async (data) => {
            ...
            if (!actual || !nueva || !repetida) { await this.errorAlert('Completá todos los campos.'); return false; }
            if (nueva.length < 6) { await this.errorAlert('La nueva contraseña debe tener al menos 6 caracteres.'); return false; }
            if (nueva !== repetida) { await this.errorAlert('Las contraseñas no coinciden.'); return false; }
